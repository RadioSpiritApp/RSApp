class UpdateImageImportWorker
  ...
  def perform()
    ...
    image_files.split(' ').each do |image_filename|
      ...
      next if image_filename == '.' or image_filename == '..' or image.blank?
      ...
      if File.exist?("public/#{image_filename}")
        ...
        image.update(attachment: image_file)
        image_update_count += 1
        notification_msg = (image_update_count % 100 == 0) ? "#{image_update_count/100 * 100} image files are updated" : " "
    ...
    notification_msg = image_files.blank? ? "No image files are present in remote server" : ""
    notification = Notification.create(notification_type: 'image_upload', message: notification_msg) if notification_msg.present?
  end
end
